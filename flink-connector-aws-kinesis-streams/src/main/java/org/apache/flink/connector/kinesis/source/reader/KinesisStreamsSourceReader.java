...
public class KinesisStreamsSourceReader<T>
    ...
    protected void onSplitFinished(Map<String, KinesisShardSplitState> finishedSplitIds) {
        if (!finishedSplitIds.isEmpty()) {
            Set<String> completedSplitIds = new HashSet<>();
            for (KinesisShardSplitState state : finishedSplitIds.values()) {
                completedSplitIds.add(state.getSplitId());
            }
            context.sendSourceEventToCoordinator(new CompletedShardsEvent(completedSplitIds));
        }
    }
